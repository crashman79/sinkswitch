...
echo "=============================================="
echo ""
echo "Recommended – run as app (no systemd required):"
echo "  1. Launch: PipeWire Audio Router (from app menu or ~/.config/pipewire-router/launch-gui.sh)"
echo "  2. In Settings tab: choose 'Run in app' and optionally 'Launch app at login' or 'Start router with systemd at session start'"
echo ""
echo "Alternative – systemd only:"
echo "  1. systemctl --user start pipewire-router"
echo "  2. systemctl --user enable pipewire-router"
echo ""
echo "GUI (PyQt6 required: sudo pacman -S python-pyqt6):"
echo "  - Config & status, Devices, Routing rules, Active streams, Logs, Settings (run mode & start on login)"
echo ""
